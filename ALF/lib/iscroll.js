...
//		that.scroller.style.top = that.y + 'px';
	...
	_start: function (e) {
		...
		if (that.options.momentum) {
			...
			if (that.options.HWCompositing) {
			    console.log(window.getComputedStyle(that.scroller, null).webkitTransform)
...
/*			if (newPosX > 0 || newPosX < that.maxScrollX || newPosY > 0 || newPosY < that.maxScrollY) {
	...
	unbind:  function() {
	   console.log('iScroll: I am unbound')
	   var that = this;
       // Remove the event listeners
       that._unbind('webkitTransitionEnd');
       that._unbind(RESIZE_EV);
	...
	rebind:  function() {
	   console.log('iScroll: I am rebound')
	   var that = this;
	   // Remove the event listeners
